...
vi.mock("@/lib/pivotaAgentBff", async () => {
  const actual = await vi.importActual<typeof import("@/lib/pivotaAgentBff")>(
    "@/lib/pivotaAgentBff",
  );
  return {
    ...actual,
    ...
    bffChatStream: vi
      .fn()
      .mockRejectedValue(new Error("stream unavailable in test")),
    sendRecoEmployeeFeedback: vi.fn(),
  };
});

vi.mock("@/lib/auroraAnalytics", async () => {
  const actual = await vi.importActual<typeof import("@/lib/auroraAnalytics")>(
    "@/lib/auroraAnalytics",
  );
  return {
    ...actual,
...
describe("BffChat product-parse degraded UX", () => {
  ...
  it("routes a bare product URL from the main composer into deep scan instead of /v1/chat", async () => {
    ...
    mock.mockImplementation((path: string) => {
      if (path === "/v1/session/bootstrap") {
        return Promise.resolve(
          makeEnvelope({
            request_id: "req_bootstrap_direct_url",
            trace_id: "trace_bootstrap_direct_url",
          }),
        );
      }
            ...
            cards: [
              {
                ...
                payload: {
                  assessment: {
                    ...
                    summary:
                      "INCI confidence is low, but watchouts are still explicit.",
                    reasons: [
                      "INCI confidence is low, but watchouts are still explicit.",
                    ],
                    anchor_product: {
                    ...
                    watchouts: [
                      {
                        ...
                        status: "possible",
                        what_to_do:
                          "Add barrier hydration and reduce frequency.",
                      },
                    ],
                  },
                  evidence: {
    ...
    const composer =
      await screen.findByPlaceholderText(/paste a product link/i);
    fireEvent.change(composer, {
      target: {
        value:
          "https://www.labseries.com/product/32020/123634/skincare/moisturizerspf/daily-rescue-energizing-lightweight-lotion-moisturizer/daily-rescue",
      },
    });
    fireEvent.submit(composer.closest("form") as HTMLFormElement);

    await waitFor(() => {
      expect(screen.getAllByText(/needs verification/i).length).toBeGreaterThan(
        0,
      );
    });

    const parseCall = mock.mock.calls.find(
      (call) => call[0] === "/v1/product/parse",
    );
    expect(parseCall).toBeTruthy();
    expect(
      JSON.parse(String((parseCall?.[2] as any)?.body || "{}")),
    ).toMatchObject({
      url: "https://www.labseries.com/product/32020/123634/skincare/moisturizerspf/daily-rescue-energizing-lightweight-lotion-moisturizer/daily-rescue",
    });
  ...
  it("hides parse-missing card when analyze returns a meaningful verdict", async () => {
    ...
    const entry = await screen.findByRole("button", {
      name: /evaluate a specific product for me/i,
    });
    ...
    await waitFor(() => {
      const analyzeCalls = mock.mock.calls.filter(
        (call) => call[0] === "/v1/product/analyze",
      );
    ...
    expect(
      screen.queryByText(/failed to parse a product entity/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/no stable product anchor was parsed/i),
    ).not.toBeInTheDocument();
    expect(vi.mocked(emitAuroraProductParseMissing)).toHaveBeenCalledTimes(1);
    expect(
      vi.mocked(emitAuroraProductParseMissing).mock.calls[0][1],
    ).toMatchObject({
      request_id: "req_parse_1",
  ...
  it("hides a low-confidence parse card once deep scan returns a usable analysis card", async () => {
                ...
                payload: {
                  ...
                  evidence: {
                    science: {
                      ...
                      fit_notes: [],
                      risk_notes: [
                        "Patch test if you are fragrance-sensitive.",
                      ],
                    },
                    social_signals: {
                    ...
                    expert_notes: [
                      "Evidence source: ingredient list parsed from product page.",
                    ],
                    missing_info: ["version_verification_needed"],
                  },
                  confidence: 0.67,
    ...
    fireEvent.change(productInput, {
      target: {
        value: "Lab Series All-In-One Defense Lotion Moisturizer SPF 35",
      },
    });
    fireEvent.click(screen.getByRole("button", { name: /^analyze$/i }));
    ...
    expect(
      screen.queryByText(/an unreliable anchor was blocked from id binding/i),
    ).toBeNull();
    expect(
      screen.queryByText(
        /this card failed to render and was safely downgraded/i,
      ),
    ).toBeNull();
  });
  ...
  it("shows degraded parse reason labels when analyze verdict remains unknown", async () => {
            ...
            cards: [
              {
                ...
                field_missing: [
                  {
                    field: "parse.fallback",
                    reason: "catalog_backend_not_configured",
                  },
                ],
              },
            ],
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/no stable product anchor was parsed/i),
      ).toBeInTheDocument();
    });
    expect(
      screen.getByText(/catalog backend is not configured/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows product-analysis diagnostic labels and evidence sources for degraded URL flow", async () => {
    ...
    mock.mockImplementation((path: string) => {
      if (path === "/v1/session/bootstrap") {
        return Promise.resolve(
          makeEnvelope({
            request_id: "req_bootstrap_3",
            trace_id: "trace_bootstrap_3",
          }),
        );
      }
      if (path === "/v1/product/parse") {
            ...
            cards: [
              {
                ...
                payload: {
                  product: {
                    ...
                    url: "https://brand.example/spf35.html",
                  },
                  confidence: 0.62,
            ...
            cards: [
              {
                ...
                payload: {
                  ...
                  evidence: {
                    ...
                    expert_notes: [
                      "Regulatory supplement loaded from DailyMed.",
                    ],
                    confidence: 0.41,
                    missing_info: ["evidence_missing"],
                    sources: [
                      ...
                      {
                        type: "inci_decoder",
                        url: "https://incidecoder.com/products/labseries-all-in-one-defense",
                        label: "INCIDecoder",
                      },
                    ],
                  },
    ...
    const productInput = await screen.findByPlaceholderText(/nivea creme/i);
    fireEvent.change(productInput, {
      target: { value: "https://brand.example/spf35.html" },
    });
    fireEvent.click(screen.getByRole("button", { name: /^analyze$/i }));
    ...
    expect(
      screen.getAllByText(
        /official page fetch was blocked by site policy \(403\)/i,
      ).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByRole("link", { name: /official page/i }),
    ).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /dailymed/i })).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /incidecoder/i }),
    ).toBeInTheDocument();
  ...
  it("shows anchor soft-block and KB quarantine diagnostics", async () => {
    ...
    mock.mockImplementation((path: string) => {
      if (path === "/v1/session/bootstrap") {
        return Promise.resolve(
          makeEnvelope({
            request_id: "req_bootstrap_4",
            trace_id: "trace_bootstrap_4",
          }),
        );
      }
      if (path === "/v1/product/parse") {
            ...
            cards: [
              {
                ...
                payload: {
                  assessment: {
                    verdict: "Unknown",
                    reasons: [
                      "The target page was blocked by site policy (403).",
                    ],
                  },
                  evidence: {
                    science: {
    ...
    const productInput = await screen.findByPlaceholderText(/nivea creme/i);
    fireEvent.change(productInput, {
      target: { value: "https://www.labseries.com/product/x" },
    });
    fireEvent.click(screen.getByRole("button", { name: /^analyze$/i }));

    await waitFor(() => {
      expect(
        screen.getByText(/an unreliable anchor was blocked/i),
      ).toBeInTheDocument();
    });
    expect(
      screen.getAllByText(
        /a stale kb hit was quarantined and recalculated in real time/i,
      ).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("renders alternatives in competitors/dupes/related sections and hides non-skincare candidates", async () => {
    ...
    mock.mockImplementation((path: string) => {
      if (path === "/v1/session/bootstrap") {
        return Promise.resolve(
          makeEnvelope({
            request_id: "req_bootstrap_5",
            trace_id: "trace_bootstrap_5",
          }),
        );
      }
      if (path === "/v1/product/parse") {
    ...
    const productInput = await screen.findByPlaceholderText(/nivea creme/i);
    fireEvent.change(productInput, {
      target: { value: "https://www.labseries.com/product/test" },
    });
    fireEvent.click(screen.getByRole("button", { name: /^analyze$/i }));
    ...
    expect(
      screen.queryByText(/s05 moisturizer brush/i),
    ).not.toBeInTheDocument();
    expect(
      screen.getByText(/how to layer \(inline guidance\)/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/advanced compatibility check/i),
    ).toBeInTheDocument();
  });
});
