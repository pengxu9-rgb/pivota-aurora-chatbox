...
describe('BffChat check-in refresh routing', () => {
  ...
  it('bypasses local reco-goal clarification for check-in refresh even without a saved goal', async () => {
    const mock = vi.mocked(bffJson);
    mock.mockImplementation((path: string) => {
      if (path === '/v1/session/bootstrap') {
        return Promise.resolve(
    ...
    expect(body?.client_state).toBe('CHECKIN_FLOW');
    expect(body?.action?.data?.reco_refresh_reason).toBe('checkin_logged');
  });
});
